...
function clearKeyGames(ky) {
    $('#bgm_h_' + ky).removeClass('g_gr_c').addClass('g_gr_n');
    $('#bgm_h_' + ky).find('span.g_home_o').removeClass('g_home_o').addClass('g_home_n');
    $('#bgm_h_' + ky).find('span.g_home_odd_o').removeClass('g_home_odd_o').addClass('g_home_odd_n');
    $('#bgm_v_' + ky).removeClass('g_gr_c').addClass('g_gr_n');
    $('#bgm_v_' + ky).find('span.g_away_o').removeClass('g_away_o').addClass('g_away_n');
    $('#bgm_v_' + ky).find('span.g_away_odd_o').removeClass('g_away_odd_o').addClass('g_away_odd_n');
    $('#bgm_m_' + ky).removeClass('g_gr_c').addClass('g_gr_n');
    $('#bgm_m_' + ky).find('span.g_odd_o').removeClass('g_odd_o').addClass('g_odd_n');
}

function toggleClickClass(ky, typ) {
    if (typ == 1) {
        if ($('#bgm_h_' + ky).hasClass('g_gr_c')) clearKeyGames(ky); else {
            clearKeyGames(ky);
            $('#bgm_h_' + ky).removeClass('g_gr_n').removeClass('g_gr_o').addClass('g_gr_c');
            $('#bgm_h_' + ky).find('span.g_home_n').removeClass('g_home_n').addClass('g_home_o');
            $('#bgm_h_' + ky).find('span.g_home_odd_n').removeClass('g_home_odd_n').addClass('g_home_odd_o');
        }
    } else if (typ == 2) {
        if ($('#bgm_v_' + ky).hasClass('g_gr_c')) clearKeyGames(ky); else {
            clearKeyGames(ky);
            $('#bgm_v_' + ky).removeClass('g_gr_n').removeClass('g_gr_o').addClass('g_gr_c');
            $('#bgm_v_' + ky).find('span.g_away_n').removeClass('g_away_n').addClass('g_away_o');
            $('#bgm_v_' + ky).find('span.g_away_odd_n').removeClass('g_away_odd_n').addClass('g_away_odd_o');
        }
    } else if (typ == 3) {
        if ($('#bgm_m_' + ky).hasClass('g_gr_c')) clearKeyGames(ky); else {
            clearKeyGames(ky);
            $('#bgm_m_' + ky).removeClass('g_gr_n').removeClass('g_gr_o').addClass('g_gr_c');
            $('#bgm_m_' + ky).find('span.g_odd_n').removeClass('g_odd_n').addClass('g_odd_o');
        }
    }
}
...
function showLog() {
    ...
    if (cartArr.length > 0) {
        $.each(cartArr, function (k, v) {
			...
			$('#picklist').append('<li class="crt_gm"><p><span class="tm"><i style="color:#f30">' + v.tn1 + '</i><i style="color:#fff"> ' + v.drto +' </i><i style="color:#006bfc">'+ v.tn2 + '</i><br><i style="color:#fc0">' + cteam + ' @' + v.rt + '</i><span><span class="odd"><a href="javascript:rmCartOn(' + k + ');" class="iconfont del">&#xe6a2;</a></span></p></li>');
            //$('#picklist').append('<li class="crt_gm"><p><span class="tm">' + v.tn1 + '</span> <a href="javascript:rmCartOn(' + k + ');" class="iconfont del">&#xe612;</a></p><p><span class="tm cho">' + v.tn2 + '</span> <span class="odd"><i class="wol">' + cteam + '</i>@ ' + v.rt + '</span></p></li>');
			//$('#picklist').append('<table width="250" height="30" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#144E8D"><tbody><tr><td><table width="100%" height="21" border="0" cellpadding="0" cellspacing="0"><tbody><tr><td style="padding-left:5px;" align="left">'+team1+' vs '+team2+'</td></tr></tbody></table><table width="250" height="21" border="0" cellpadding="0" cellspacing="0" bgcolor="#144E8D"><tbody><tr><td colspan="2" align="left" style="padding-left:2px"><table border="0" width="98%" bgcolor="#111111" height="30"><tbody><tr><td width="120px" class="cart_txt"><b>'+cteam+'</b></td><td width="20px" class="cart_txt"><b>'+chch+'</b></td><td width="50px" class="cart_txt1"><b>'+v.rt+'</b></td><td width="20px"><img src="../images/btn_bet_del.png" width="18" height="17" onclick="rmCartOn('+k+')" style="cursor:pointer"></td></tr></tbody></table></td></tr><tr height="10"><td></td></tr></tbody></table></td></tr></tbody></table><table height="2"><tbody><tr><td></td></tr></tbody></table>');
        });
...
function cartadd(ky, typ, rk) {
    ...
    if (cartArr.length > 0) {
        ...
        * */
		...
		if ($('#bgm_h_' + ky).attr('jj') == 'jong_4') {

            if (tk == 'H') {
                $.each(cartArr, function (k, v) {
                    if (v.tk == 'U' && v.tt == otname && v.gky != ky && lgk == v.lgk) isOk = false;
                });
            }
            else if (tk == 'U') {
                $.each(cartArr, function (k, v) {
                    if (v.tk == 'H' && v.tt == otname && v.gky != ky && lgk == v.lgk) isOk = false;
                });
            }
            if (isOk === false) {
                alert('같은경기의 핸디와 언오버를 동시에 픽하실 수 없습니다1.');
                return;
            }
        }
