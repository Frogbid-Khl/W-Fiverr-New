...
function getGameInfo(t) {
    ...
    resetMoneyBet();
    $.ajax({
        url: '/game/bubble_loader/game_info.php',
        type: 'GET',
        dataType: 'json',
        data: {},
        success: function (data, textStatus, jqXHR) {
...
function time_count() {
    ...
    $("#fliptimer_min2up").html(mm);
    $("#fliptimer_min2down").html(mm);
    var str= ss.toString()
    $("#fliptimer_sec1up").html(str[0]);
    $("#fliptimer_sec1down").html(str[0]);
    $("#fliptimer_sec2up").html(str[1]);
    $("#fliptimer_sec2down").html(str[1]);
...
function get_bet_list(r, r1) {
    var d = new Date();
    $.ajax({
        url: '/game/bubble_loader/search_bet_info.php?r=' + r + '&_t=' + d.getTime() + '&game_type=4',
        type: 'GET',
        dataType: 'json',
        data: {},
        success: function (data, textStatus, jqXHR) {
    ...
    //tabs = "<a href=\"javascript:get_bet_list(" + (parseInt(r) - 1) + "," + (parseInt(r1) - 1) + ");\"><img src=\"/game/bubble_loader/images/cart_arrow_left.png\"></a>";
    //tabs = tabs + "<a href=\"javascript:get_bet_list(" + (parseInt(r) + 1) + "," + (parseInt(r1) + 1) + ");\"><img src=\"/game/bubble_loader/images/cart_arrow_right.png\"></a>";
...
function check_bet(kind) {
    ...
    var choice = "";
    if (bet.kind == 'peven') {
        choice = "<div class=\"po_color1\">좌</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 1;
        bet.gameType = 1;
    } else if (bet.kind == 'podd') {
        choice = "<div class=\"po_color2\">우</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 2;
        bet.gameType = 1;
        ...
        choice = "<div class=\"po_color1\">홀</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 5;
        bet.gameType = 1;
    } else if (bet.kind == 'aodd') {
        choice = "<div class=\"po_color2\">짝</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 6;
        bet.gameType = 1;
...
function Betting() {
    ...
    $.ajax({
	url: '/game/bubble_loader/betting_sudda.php',
        type: 'POST',
        dataType: 'json',
        data: bet,
