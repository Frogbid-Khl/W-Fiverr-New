...
function getGameInfo(t) {
    ...
    resetMoneyBet();
    $.ajax({
        url: '/game/powerball/game_info.php',
        type: 'GET',
        dataType: 'json',
        data: {},
        success: function (data, textStatus, jqXHR) {
...
function get_bet_list(r, r1) {
    var d = new Date();
    $.ajax({
        url: '/game/powerball/search_bet_info.php?r=' + r + '&_t=' + d.getTime() + '&game_type=4',
        type: 'GET',
        dataType: 'json',
        data: {},
        success: function (data, textStatus, jqXHR) {
    ...
    //tabs = "<a href=\"javascript:get_bet_list(" + (parseInt(r) - 1) + "," + (parseInt(r1) - 1) + ");\"><img src=\"/game/powerball/images/cart_arrow_left.png\"></a>";
    //tabs = tabs + "<a href=\"javascript:get_bet_list(" + (parseInt(r) + 1) + "," + (parseInt(r1) + 1) + ");\"><img src=\"/game/powerball/images/cart_arrow_right.png\"></a>";
...
function check_bet(kind) {
    ...
    var choice = "";
    if (bet.kind == 'peven') {
        choice = "<span class=\"cart_list_btn_2\">파워볼</span><div class=\"po_color1\">홀</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 1;
        bet.gameType = 1;
    } else if (bet.kind == 'podd') {
        choice = "<span class=\"cart_list_btn_2\">파워볼</span><div class=\"po_color2\">짝</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 2;
        bet.gameType = 1;
    } else if (bet.kind == 'pdown') {
        choice = "<span class=\"cart_list_btn_2\">파워볼</span><div class=\"po_color1\">언더</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 3;
        bet.gameType = 1;
    } else if (bet.kind == 'pup') {
        choice = "<span class=\"cart_list_btn_2\">파워볼</span><div class=\"po_color2\">오버</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 4;
        bet.gameType = 1;
    } else if (bet.kind == 'aeven') {
        choice = "<span class=\"cart_list_btn_1\">일반볼</span><div class=\"po_color1\">홀</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 5;
        bet.gameType = 2;
    } else if (bet.kind == 'aodd') {
        choice = "<span class=\"cart_list_btn_1\">일반볼</span><div class=\"po_color2\">짝</div><span style=\"color: #98a3ae; float: left\">" + bet.odds + "</span>";
        bet.value = 6;
        bet.gameType = 2;
...
function Betting() {
    ...
    if (parseInt(left_time) <= 20) {
        ShowToast("error", "베팅 실패", "배팅가능한시간이 아닙니다");
        return;
    }

    $.ajax({
	url: '/game/powerball/betting_sudda.php',
        type: 'POST',
        dataType: 'json',
        data: bet,
