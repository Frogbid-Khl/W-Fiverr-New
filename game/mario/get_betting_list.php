...
if($member_id){
//$sql2 = "select count(*)coul from Mario_betlist where bet_uid = '".$member_id."'";
}else{
//$sql2 = "select count(*)coul from Mario_betlist";
}
$row2 = sql_fetch($sql2);
...
$sql = " select * from Mario_betlist where bet_uid = '".$member_id."' order by sid desc limit ".$pagw.", 15";
...
$countn = count($list);
   ...
   $countn = 5;
      ...
      include_once('./_common.php');
         ...
         $value = 1.95;
         $ifmoney = $money * $value;
         $url = 'https://bepick.net/game/default/jw_supermario';
         $data = webdata($url, 'GET', '');
         $data2 = splits($data, '</span><strong>'.$round.'</strong>', '</tr>', 1);
         $_count = explode('<span class="r_icon round15 jw_supermario_fd', $data2);
         $_count = count($_count);
         
         $Result1 = splits($data2,'<span class="r_icon round15 jw_supermario_fd','"', 1);
         $Result1 = str_replace('1_1', '1', $Result1);
         $Result1 = str_replace('1_2', '2', $Result1);
         $Result2 = splits($data2,'<span class="r_icon round15 jw_supermario_fd','"', 2);
         ...
         $Result3 = splits($data2,'<span class="r_icon round15 jw_supermario_fd','"', 3);
         $Result3 = str_replace('3_1', '7', $Result3);
         $Result3 = str_replace('3_2', '8', $Result3);
         $Result4 = splits($data2,'<span class="r_icon round15 jw_supermario_fd','"', 4);
	    ...
	    $Result8 = $money*1.95;
                  $Result5 = '적중';
	    insert_point( $member[ 'mb_id' ], $money*1.95, '[마리오] 적중 포인트');
                  $sql = "update Mario_betlist set bet_result = '적중', bet_ifmoney = '".$money*1.95."'  where sid = '".$sid."'";
                  sql_query($sql);
	    ...
	    $Result8 = $money*3.95;
                  $Result5 = '적중';
	    insert_point( $member[ 'mb_id' ], $money*1.95, '[마리오] 적중 포인트');
                  $sql = "update Mario_betlist set bet_result = '적중', bet_ifmoney = '".$Result8."'  where sid = '".$sid."'";
                  sql_query($sql);
	    ...
	    $Result8 = '0';
            }else 
            {
               $Result5 = '미적';
	 $Result8 = '0';
               $sql = "update Mario_betlist set bet_result = '미적' where sid = '".$sid."'";
               sql_query($sql);
            }
         } 
